...
function oneOfTypeShim(types) {
  const shim = baseShim(value => {
    ...
    for (let i = 0; i < types.length; i++) {
      const result = types[i](value);
      if (result === true) {
        // If at least one condition passed, return true now
        return true;
      } else {
        // At least one condition failed, but we don't know if others may pass
        // save this error and continue
