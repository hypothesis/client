...
import { combineGroups, WORLD_GROUP_ID } from '../helpers/groups';
...
export class GroupsService {
  ...
  private async _filterGroups(
    ...
    const pageHasAssociatedGroups = groups.some(
      g => g.id !== WORLD_GROUP_ID && g.isScopedToUri,
    );
    ...
    if (
      directLinkedGroupId === WORLD_GROUP_ID ||
      directLinkedAnnotationGroupId === WORLD_GROUP_ID
    ) {
      return groups;
    }

    // Return non-world groups.
    return groups.filter(g => g.id !== WORLD_GROUP_ID);
  }
