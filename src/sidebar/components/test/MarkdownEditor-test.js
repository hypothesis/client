...
describe('MarkdownEditor', () => {
  ...
  context('when @mentions are enabled', () => {
    ...
    function keyDownInTextarea(wrapper, key) {
      ...
      act(() =>
        textareaDOMNode.dispatchEvent(new KeyboardEvent('keydown', { key })),
      );
      wrapper.update();
    }
    ...
    [
      ...
      { usersForSuggestions: [], text: '@', expectedSuggestions: 0 },
    ...
    ].forEach(({ usersForSuggestions, text, expectedSuggestions }) => {
      it('resolves expected suggestions when based on active mention matches', () => {
        const wrapper = createComponent({
          mentionsEnabled: true,
          usersForSuggestions,
        });
        ...
        assert.equal(
          wrapper.find('MentionSuggestionsPopover').prop('suggestions').length,
          expectedSuggestions,
        );
      });
    });
