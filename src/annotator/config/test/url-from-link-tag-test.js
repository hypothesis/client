...
describe('annotator/config/url-from-link-tag', () => {
  ...
  context('incorrect link tag', () => {
    ['html', 'javascript'].forEach(type => {
      it('throws an error if link tag is missing', async () => {
        assert.throws(() => {
          urlFromLinkTag(window, 'fakeApp', type);
        }, `No application/annotator+${type} (rel="fakeApp") link in the document`);
      });

      it('throws an error if href is missing in link tag', async () => {
