...
describe('annotator/util/draw-tool-position', () => {
  ...
  describe('clampRectToViewport', () => {
    ...
    it('clamps rect when it extends beyond maxRight', () => {
      const rect = {
        type: 'rect',
        left: 80,
        top: 10,
        right: 150, // extends beyond maxRight of 100
        bottom: 50,
      };
      const viewport = {
        minLeft: 0,
      ...
      const r = clampRectToViewport(rect, viewport);
      // width = 150 - 80 = 70
      // left should be clamped to maxRight - width = 100 - 70 = 30
      assert.equal(r.left, 30);
      assert.equal(r.right, 100);
      assert.equal(r.top, 10);
      assert.equal(r.bottom, 50);
    });

    it('clamps rect when it extends beyond maxBottom', () => {
      const rect = {
        type: 'rect',
        left: 10,
        top: 150,
        right: 50,
        bottom: 250, // extends beyond maxBottom of 200
      };
      const viewport = {
        minLeft: 0,
        minTop: 0,
      ...
      const r = clampRectToViewport(rect, viewport);
      // height = 250 - 150 = 100
      // top should be clamped to maxBottom - height = 200 - 100 = 100
      assert.equal(r.top, 100);
      assert.equal(r.bottom, 200);
      assert.equal(r.left, 10);
      assert.equal(r.right, 50);
    });

    it('clamps rect when it extends beyond both maxRight and maxBottom', () => {
      const rect = {
        type: 'rect',
        left: 80,
        top: 150,
      ...
      const r = clampRectToViewport(rect, viewport);
      // width = 150 - 80 = 70, height = 250 - 150 = 100
      // left should be clamped to maxRight - width = 100 - 70 = 30
      // top should be clamped to maxBottom - height = 200 - 100 = 100
      assert.equal(r.left, 30);
      assert.equal(r.right, 100);
