...
function loadStyles(shadowRoot: ShadowRoot) {
  ...
  // Add @property declarations to main document for properties referenced
  // by the Shadow DOM stylesheet.
  if (propertyStyleSheet === undefined) {
    propertyStyleSheet = new CSSStyleSheet();
    propertyStyleSheet.replace(tailwindProperties);
    document.adoptedStyleSheets.push(propertyStyleSheet);
  }
}
...
// Tailwind properties extracted from compiled annotator.css as follows:
//
// 1. Build CSS using `gulp build-css`
// 2. Copy all the @property rules from `build/styles/annotator.css`
// 3. Paste into string literal below
//
// We assume we can get away with this on the basis that the properties will
// change rarely.
