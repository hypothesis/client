...
import type { Anchor, AnnotationData, AnnotationTool, Annotator, Integration, SidebarLayout } from '../../types/annotator';
...
export class BookReaderIntegration extends TinyEmitter implements Integration {
  ...
  constructor(annotator: Annotator) {
    ...
    this._debouncedUpdate = debounce(this._update, 100, {leading: false, trailing: true});
  ...
  _update = async () => {
    ...
    for (const anchor of this._annotator.anchors) {
      ...
      const placeholder = anchor.highlights[0].closest('.BRannotationPlaceholder');
      // If it's a placeholder and its page is now visible, we want to re-anchor
      if (placeholder) {
        const pageSelector = anchor.target.selector?.find(s => s.type === 'PageSelector') as PageSelector;
        if (!pageSelector) {
          throw new Error('No page selector found');
        }

        const pageContainer = this._br.refs.$br.find(`.BRpagecontainer[data-index="${pageSelector.index}"]`);
      ...
      const notInDom = anchor.highlights.some((highlight) => !document.body.contains(highlight));
      if (notInDom) {
        delete anchor.region;
        // Un-annotate
